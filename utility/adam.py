...
class Adam(tf.keras.optimizers.Optimizer):
    ...
    def _resource_apply_dense(self, grad, var):
        ...
        m.assign(m_t, use_locking=self._use_locking)
        m_corr_t = m_t / (1.0 - beta_1_power)

        v_t = beta_2_t * v + (1.0 - beta_2_t) * tf.math.square(grad)
        v.assign(v_t, use_locking=self._use_locking)
        ...
        self._var_grad_map[var.name] = transformed_grads = -lr_t * var_t

        var_update = var.assign_add(transformed_grads, use_locking=self._use_locking)
